...
public class StylingFeeFAQMenuOptions {
		...
		public void temp10() {
	        ...
	        List <WebElement> tmpHSFX = objFAQ.getHeaderStylingFeeX();
	        
	        for(int i = 0; i < tmpHSFX.size(); i++){
	        	
	        	if(tmpHSFX.get(i).isDisplayed()) {
	        		tmpHSFX.get(i).click();   
	        		System.out.println("Element was found: " + tmpHSFX.get(i).getText().toString());
	        	}else {
	        		tmpHSFX.get(0).click();
	        		//testDriverFF.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
	        		if(tmpHSFX.get(i).isDisplayed()) {
	        			tmpHSFX.get(i).click();
	        			System.out.println("Element was found after Retry: " + tmpHSFX.get(i).getText().toString());
	        		}else {
	        			System.out.println("Element was NOT clickable after Retry: " + tmpHSFX.get(i).getText().toString());
	        		}
	        		
	        	}
	        }
	        
	        for(int i = (tmpHSFX.size()-1); i >= 0; i--){
