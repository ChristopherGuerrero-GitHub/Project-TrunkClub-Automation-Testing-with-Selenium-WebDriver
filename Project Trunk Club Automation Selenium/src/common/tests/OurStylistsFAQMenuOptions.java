...
public class OurStylistsFAQMenuOptions {
	...
	public void temp12() {
	    ...
	    List <WebElement> tmpHOSX = objFAQ.getHeaderOurStylistX();
	    
	    for(int i = 0; i < tmpHOSX.size(); i++){
	    		    	
	    	if(tmpHOSX.get(i).isDisplayed()) {
	    		tmpHOSX.get(i).click();   
        		System.out.println("Element was found: " + tmpHOSX.get(i).getText().toString());
        	}else {
        		tmpHOSX.get(0).click();
        		//testDriverFF.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
        		if(tmpHOSX.get(i).isDisplayed()) {
        			tmpHOSX.get(i).click();
        			System.out.println("Element was found after Retry: " + tmpHOSX.get(i).getText().toString());
        		}else {
        			System.out.println("Element was NOT clickable after Retry: " + tmpHOSX.get(i).getText().toString());
        		}
	    ...
	    for(int i = (tmpHOSX.size()-1); i >= 0; i--){
