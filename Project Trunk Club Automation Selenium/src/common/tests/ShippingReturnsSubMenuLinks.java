...
public class ShippingReturnsSubMenuLinks {
		...
		public void temp9() {
	        ...
	        List <WebElement> tmpHSRL = objFAQ.getHeaderShipReturnLinkX();
	        
	        for(int i = 0; i < tmpHSRL.size(); i++){
	        	
	        	if(tmpHSRL.get(i).isDisplayed()) {
	        		tmpHSRL.get(i).click();   
	        		System.out.println("Element was found: " + tmpHSRL.get(i).getText().toString());
	        	}else {
	        		tmpHSRL.get(0).click();
	        		//testDriverFF.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
	        		if(tmpHSRL.get(i).isDisplayed()) {
	        			tmpHSRL.get(i).click();
	        			System.out.println("Element was found after Retry: " + tmpHSRL.get(i).getText().toString());
	        		}else {
	        			System.out.println("Element was NOT clickable after Retry: " + tmpHSRL.get(i).getText().toString());
	        		}
