...
public class AlterationsFAQMenuOptions {
	    ...
	    List <WebElement> tmpHAX = objFAQ.getHeaderAlterationsX();
	    
	    for(int i = 0; i < tmpHAX.size(); i++){
	    	
	    	if(tmpHAX.get(i).isDisplayed()) {
	    		tmpHAX.get(i).click();   
        		System.out.println("Element was found: " + tmpHAX.get(i).getText().toString());
        	}else {
        		tmpHAX.get(0).click();
        		//testDriverFF.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
        		if(tmpHAX.get(i).isDisplayed()) {
        			tmpHAX.get(i).click();
        			System.out.println("Element was found after Retry: " + tmpHAX.get(i).getText().toString());
        		}else {
        			System.out.println("Element was NOT clickable after Retry: " + tmpHAX.get(i).getText().toString());
        		}
        		
        	}
	    }
	    
	    for(int i = (tmpHAX.size()-1); i >= 0; i--){
