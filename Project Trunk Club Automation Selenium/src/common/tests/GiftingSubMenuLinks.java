...
public class GiftingSubMenuLinks {
	...
	public void temp23() {
	    ...
	    List <WebElement> tmpHGL = objFAQ.getHeaderGiftingLinkX();
	    
	    for(int i = 0; i < tmpHGL.size(); i++){
	    	
	    	if(tmpHGL.get(i).isDisplayed()) {
	    		tmpHGL.get(i).click();   
        		System.out.println("Element was found: " + tmpHGL.get(i).getText().toString());
        	}else {
        		tmpHGL.get(0).click();
        		//testDriverFF.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
        		if(tmpHGL.get(i).isDisplayed()) {
        			tmpHGL.get(i).click();
        			System.out.println("Element was found after Retry: " + tmpHGL.get(i).getText().toString());
        		}else {
        			System.out.println("Element was NOT clickable after Retry: " + tmpHGL.get(i).getText().toString());
        		}
