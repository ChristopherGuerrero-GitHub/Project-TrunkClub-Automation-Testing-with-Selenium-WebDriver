...
public class PricingSubMenuLinks {
	...
	public void temp5() {
        ...
        List <WebElement> tmpHPL = objFAQ.getHeaderPricingLinkX();
        
        for(int i = 0; i < tmpHPL.size(); i++){
        	
        	if(tmpHPL.get(i).isDisplayed()) {
        		tmpHPL.get(i).click();   
        		System.out.println("Element was found: " + tmpHPL.get(i).getText().toString());
        	}else {
        		//testDriverFF.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
        		tmpHPL.get(0).click();
        		if(tmpHPL.get(i).isDisplayed()) {
        			tmpHPL.get(i).click();
        			System.out.println("Element was found after Retry: " + tmpHPL.get(i).getText().toString());
        		}else {
        			System.out.println("Element was NOT clickable after Retry: " + tmpHPL.get(i).getText().toString());
        		}
        	}
        }
