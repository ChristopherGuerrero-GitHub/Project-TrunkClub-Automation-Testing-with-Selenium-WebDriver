...
public class TrunksSubMenuLinks {
		...
		public void temp17() {
		    ...
		    List <WebElement> tmpHTL = objFAQ.getHeaderTrunksLinkX();
		    
		    for(int i = 0; i < tmpHTL.size(); i++){
		    			    	
		    	if(tmpHTL.get(i).isDisplayed()) {
		    		tmpHTL.get(i).click();   
	        		System.out.println("Element was found: " + tmpHTL.get(i).getText().toString());
	        	}else {
	        		tmpHTL.get(0).click();
	        		//testDriverFF.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
	        		if(tmpHTL.get(i).isDisplayed()) {
	        			tmpHTL.get(i).click();
	        			System.out.println("Element was found after Retry: " + tmpHTL.get(i).getText().toString());
	        		}else {
	        			System.out.println("Element was NOT clickable after Retry: " + tmpHTL.get(i).getText().toString());
	        		}
