...
public class TrunksFAQMenuOptions {
	...
	public void temp16() {
	    ...
	    List <WebElement> tmpHTX = objFAQ.getHeaderTrunksX();
	    
	    for(int i = 0; i < tmpHTX.size(); i++){
	    	
	    	if(tmpHTX.get(i).isDisplayed()) {
	    		tmpHTX.get(i).click();   
        		System.out.println("Element was found: " + tmpHTX.get(i).getText().toString());
        	}else {
        		tmpHTX.get(0).click();
        		//testDriverFF.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
        		if(tmpHTX.get(i).isDisplayed()) {
        			tmpHTX.get(i).click();
        			System.out.println("Element was found after Retry: " + tmpHTX.get(i).getText().toString());
        		}else {
        			System.out.println("Element was NOT clickable after Retry: " + tmpHTX.get(i).getText().toString());
        		}
        		
        	}
	    }
	    
	    for(int i = (tmpHTX.size()-1); i >= 0; i--){
