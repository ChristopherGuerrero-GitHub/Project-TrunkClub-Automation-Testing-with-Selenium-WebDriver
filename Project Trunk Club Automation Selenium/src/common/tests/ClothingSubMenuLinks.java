...
public class ClothingSubMenuLinks {
	        ...
	        List <WebElement> tmpHCL = objFAQ.getHeaderClothingLinkX();
	        
	        for(int i = 0; i < tmpHCL.size(); i++){
	        		        	
	        	if(tmpHCL.get(i).isDisplayed()) {
	        		tmpHCL.get(i).click();   
	        		System.out.println("Element was found: " + tmpHCL.get(i).getText().toString());
	        	}else {
	        		tmpHCL.get(0).click();
	        		//testDriverFF.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
	        		if(tmpHCL.get(i).isDisplayed()) {
	        			tmpHCL.get(i).click();
	        			System.out.println("Element was found after Retry: " + tmpHCL.get(i).getText().toString());
	        		}else {
	        			System.out.println("Element was NOT clickable after Retry: " + tmpHCL.get(i).getText().toString());
	        		}
