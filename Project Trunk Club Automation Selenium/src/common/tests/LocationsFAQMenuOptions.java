...
public class LocationsFAQMenuOptions {
		...
		public void temp20() {
		    ...
		    List <WebElement> tmpHLX = objFAQ.getHeaderLocationsX();
		    
		    for(int i = 0; i < tmpHLX.size(); i++){
		    	
		    	if(tmpHLX.get(i).isDisplayed()) {
		    		tmpHLX.get(i).click();   
	        		System.out.println("Element was found: " + tmpHLX.get(i).getText().toString());
	        	}else {
	        		tmpHLX.get(0).click();
	        		//testDriverFF.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
	        		if(tmpHLX.get(i).isDisplayed()) {
	        			tmpHLX.get(i).click();
	        			System.out.println("Element was found after Retry: " + tmpHLX.get(i).getText().toString());
	        		}else {
	        			System.out.println("Element was NOT clickable after Retry: " + tmpHLX.get(i).getText().toString());
	        		}
	        		
	        	}
		    }
		    
		    for(int i = (tmpHLX.size()-1); i >= 0; i--){
