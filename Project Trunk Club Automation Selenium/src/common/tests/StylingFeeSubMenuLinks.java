...
public class StylingFeeSubMenuLinks {
	...
	public void temp11() {
	    ...
	    List <WebElement> tmpHSFL = objFAQ.getHeaderStylingFeeLinkX();
	    
	    for(int i = 0; i < tmpHSFL.size(); i++){
	    		    	
	    	if(tmpHSFL.get(i).isDisplayed()) {
	    		tmpHSFL.get(i).click();   
        		System.out.println("Element was found: " + tmpHSFL.get(i).getText().toString());
        	}else {
        		tmpHSFL.get(0).click();
        		//testDriverFF.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
        		if(tmpHSFL.get(i).isDisplayed()) {
        			tmpHSFL.get(i).click();
        			System.out.println("Element was found after Retry: " + tmpHSFL.get(i).getText().toString());
        		}else {
        			System.out.println("Element was NOT clickable after Retry: " + tmpHSFL.get(i).getText().toString());
        		}
        	}
