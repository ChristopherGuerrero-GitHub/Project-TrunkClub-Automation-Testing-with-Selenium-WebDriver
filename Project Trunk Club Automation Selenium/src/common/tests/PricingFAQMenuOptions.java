...
public class PricingFAQMenuOptions {
	...
	public void temp4() {
        ...
        List <WebElement> tmpHPX= objFAQ.getHeaderPricingX();
        
        for(int i = 0; i < tmpHPX.size(); i++){
        	
        	if(tmpHPX.get(i).isDisplayed()) {
        		tmpHPX.get(i).click();   
        		System.out.println("Element was found: " + tmpHPX.get(i).getText().toString());
        	}else {
        		tmpHPX.get(0).click();
        		//testDriverFF.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
        		if(tmpHPX.get(i).isDisplayed()) {
        			tmpHPX.get(i).click();
        			System.out.println("Element was found after Retry: " + tmpHPX.get(i).getText().toString());
        		}else {
        			System.out.println("Element was NOT clickable after Retry: " + tmpHPX.get(i).getText().toString());
        		}
