...
public class GiftingFAQMenuOptions {
		...
		public void temp22() {
		    ...
		    List <WebElement> tmpHGX = objFAQ.getHeaderGiftingX();
		    
		    for(int i = 0; i < tmpHGX.size(); i++){
		    	
		    	if(tmpHGX.get(i).isDisplayed()) {
		    		tmpHGX.get(i).click();   
	        		System.out.println("Element was found: " + tmpHGX.get(i).getText().toString());
	        	}else {
	        		tmpHGX.get(0).click();
	        		//testDriverFF.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
	        		if(tmpHGX.get(i).isDisplayed()) {
	        			tmpHGX.get(i).click();
	        			System.out.println("Element was found after Retry: " + tmpHGX.get(i).getText().toString());
	        		}else {
	        			System.out.println("Element was NOT clickable after Retry: " + tmpHGX.get(i).getText().toString());
	        		}
	        		
	        	}
		    }
		    
		    for(int i = (tmpHGX.size()-1); i >= 0; i--){
