...
public class LocationsSubMenuLinks {
	...
	public void temp21() {
	    ...
	    List <WebElement> tmpHLL = objFAQ.getHeaderLocationsLinkX();
	    
	    for(int i = 0; i < tmpHLL.size(); i++){
	    		    	
	    	if(tmpHLL.get(i).isDisplayed()) {
	    		tmpHLL.get(i).click();   
        		System.out.println("Element was found: " + tmpHLL.get(i).getText().toString());
        	}else {
        		tmpHLL.get(0).click();
        		//testDriverFF.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
        		if(tmpHLL.get(i).isDisplayed()) {
        			tmpHLL.get(i).click();
        			System.out.println("Element was found after Retry: " + tmpHLL.get(i).getText().toString());
        		}else {
        			System.out.println("Element was NOT clickable after Retry: " + tmpHLL.get(i).getText().toString());
        		}
